 ...
 */
@RequiresApi(api = 21)
public class Camera2Impl extends CameraImpl {
    ...
    private CameraListener mCameraCallbace;
     ...
     * {@link TextureView}.
     */
    private final PreviewImpl.Callback mSurfaceTextureListener = new PreviewImpl.Callback() {
        @Override
        public void onSurfaceChanged() {
            if (mTextureView.isReady()) {
                openCamera(mActivity,mTextureView.getWidth(), mTextureView.getHeight());
            }
        }
    };
     ...
     */
    private PreviewImpl mTextureView;
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            buffer.get(bytes);
            mCameraCallbace.onPictureTaken(bytes);
    ...
    Camera2Impl(CameraListener callback, PreviewImpl preview) {
        super(callback, preview);
        mCameraCallbace = callback;
        if (preview != null && preview.getView() != null) {
            Context context = preview.getView().getContext();
            ...
            if (mActivity == null) {
                throw new IllegalArgumentException("not found activity context");
            }
    ...
    void setFacing(@Facing int facing) {
        ...
        try {
            for (String s : mCameraManager.getCameraIdList()) {
                ...
                Integer intelFacing = mCameraCharacteristics.get(CameraCharacteristics.LENS_FACING);
                if (intelFacing != null) {
                    if (facing == CameraKit.Constants.FACING_FRONT && intelFacing == CameraCharacteristics.LENS_FACING_FRONT) {
                        mCameraId = s;
                        break;
                    }else if(facing == CameraKit.Constants.FACING_BACK && intelFacing == CameraCharacteristics.LENS_FACING_BACK){
                        mCameraId = s;
                        break;
                    }else{
                        continue;
    ...
    public Size translateToJavaSize(android.util.Size size){
        Size result = new Size(size.getWidth(), size.getHeight());
        return result;
    }
    /**
   ...
   public void setSurfar(TextureViewPreview view){
       mTextureView = view;
   }
   ...
   @Override
    public void start() {
        ...
        if (mTextureView.isReady()) {
            ...
            mTextureView.setCallback(mSurfaceTextureListener);
        }
    }
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
                    mTextureView.setTruePreviewSize(mPreviewSize.getWidth(), mPreviewSize.getHeight());
                } else {
                    mTextureView.setTruePreviewSize(
    ...
    private void openCamera(Activity activity,int width, int height) {
        ...
        try {
            ...
            mCameraCallbace.onCameraOpened();
            isCameraOpened = true;
        } catch (CameraAccessException e) {
            e.printStackTrace();
            mCameraCallbace.onCameraError("",e);
                    ...
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                            ...
                            try {
                                ...
                                e.printStackTrace();
                                mCameraCallbace.onCameraError("configurefailed",null);
                            }
                        }
...
//    }
    ...
    private void takeStandardPicture() {
        lockStandardFocus();
    }

    private void lockStandardFocus() {
     ...
     */
    private void captureStillPicture(Activity activity) {
        try {
